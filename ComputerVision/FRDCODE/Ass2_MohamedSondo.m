...
colormap(MAP);
figure();
image(intensity2),title('intensity of Pepper picture');
colormap(MAP);
...
image(comb_one2),title('herizonital, verital, and combined of 1x2 operator of Peppers');
colormap(MAP);
...
%for sobel operation for Peppers
dx_sobel2 = (zeros(R, C));
dy_sobel2 = (zeros(R, C));
sobel_XY2= (zeros(R,C));
...
image(sobel_images),title(' Peppers sobel with x, sobel with y and combine sobel XY');
colormap(MAP);
...
figure();
image(residuals2),title(' residuals of 1X2 and sobel operator');
colormap(MAP);
