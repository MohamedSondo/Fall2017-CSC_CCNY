...
(define let-vars
  (lambda (e)
    (cond ((null? e) '())
          (else (cons (car (car e)) (let-vars (cdr e)))))))

(define let-values
  (lambda (e)
    (cond ((null? e) '())
          (else (cons (car (cdr (car e))) (let-values (cdr e)))))))
...
(define *let
  (lambda (e table)
    ; converts (let ((x1 v1) (x2 v2) ...) BODY) into ((lambda (x1 x2 ...) BODY) v1 v2
    (meaning (cons (list 'lambda (let-vars (let-bindings e))  (let-body e)) (let-values (let-bindings e))) table)))

; (let* ((x1 v1) (x2 v2) (x3 x1) ... ) BODY)
; becomes
; (let ((x1 v1)) (let ((x2 v2)) (let ((x3 x1)) BODY)
(define expand-let*
  (lambda (bindings body)
    (cond ((null? bindings) body)
          (else (list 'let (list (car bindings)) (expand-let* (cdr bindings) body))))))

(define *let-iter
  (lambda (e table)
    (meaning (expand-let* (let-bindings e) (let-body e)) table)))

; (if (question) true-answer false-answer)
(define (third l)
  (car (cdr (cdr l))))
...
(define *if
  (lambda (e table)
    (meaning (list 'cond (list (if-question e) (true-answer e)) (list 'else (false-answer e))) table)))

(value '((lambda (x) (let ((y (quote (3))))
                        (cons x y))) 2))

(value '((lambda (x) (let ((y 4) (z 5))
                       (cons y (cons z x)))) (quote ())))

(value '((lambda (x) (let* ((x1 2) (x2 3) (x3 x1)) (+ (+ (+ x1 x2) x3) x))) 3)) ; 10
(value '((lambda (x) (if (zero? x) 'zero! 'not-zero!)) 0))
(value '((lambda (x) (if (zero? x) 'zero! 'not-zero!)) 5))